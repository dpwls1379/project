...
$(function () {
	$('#buy').click(function() {
		...
		if(cf){
			...
			$('#buy_delidate').attr('value',date);
			date=$('#buy_delidate').val();
			frm.submit();
		}else{
			return false;
		}
...
<%-- 		<input type="hidden" name="id" value="${id }"> --%>
		...
		<input type="hidden" name="buy_delidate" id="buy_delidate">
		...
		<table class="table table-striped">
			...
			<c:forEach var="info" items="${info }">
				<tr>
					<td><img alt="" src="images/${info.pro_image }" class="img-sm"></td>
					<td>${info.pro_name }</td>
